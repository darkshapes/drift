...
use drift_proto::{DriftMessage, NodeInfo, TrainConfig, read_message, write_message, DRIFT_ALPN};
use drift_auth::crypto::verify_repo_commit;

#[tokio::test]
async fn test_repo_commit_signed_with_iroh_key() {
    let node_endpoint = create_endpoint().await.unwrap();
    let node_id = node_endpoint.id().to_string();
    let node_pubkey = node_endpoint.id();

    let coord_endpoint = create_endpoint().await.unwrap();
    ...
    let coord_task = tokio::spawn(async move {
        let conn = coord_endpoint.connect(node_pubkey_for_coord, DRIFT_ALPN).await.unwrap();
        ...
        write_message(&mut send, &DriftMessage::TrainComplete).await.unwrap();
    });
